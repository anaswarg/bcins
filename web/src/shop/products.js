export default {
  "policy": [
    {
      "brand": "Appliance Coverage",
      "model": "One Year",
      "price": 1000.0,
      "imgSrc": "/img/products/appliance.png",
      "description": `Upto 5 individual appliances of your choice`
    },
    {
      "brand": "Systems Coverage",
      "model": "One Year",
      "price": 2000.0,
      "imgSrc": "/img/products/system.png",
      "description": `5 major systems - electrical, plumbing, duct etc`
    },
    {
      "brand": "Combo Coverage",
      "model": "One Year",
      "price": 2500.0,
      "imgSrc": "/img/products/combo.png",
      "description": `Appliance + Systems coverage`
    }
  ],
  "bikes": [
    {
      "brand": "Appliance Coverage",
      "model": "One Year",
      "price": 1000.0,
      "imgSrc": "/img/products/appliance.png",
      "description": `Upto 5 individual appliances of your choice`
    },
    {
      "brand": "Systems Coverage",
      "model": "One Year",
      "price": 2000.0,
      "imgSrc": "/img/products/system.png",
      "description": `5 major systems - electrical, plumbing, duct etc`
    },
    {
      "brand": "Combo Coverage",
      "model": "One Year",
      "price": 2500.0,
      "imgSrc": "/img/products/combo.png",
      "description": `Appliance + Systems coverage`
    }
  ],
  "smart-phones": [
    {
      "brand": "Samsung",
      "model": "S7",
      "price": 660,
      "imgSrc": "/img/products/smartPhone1.png",
      "description": `Colour: Black <br> Memory: 32GB`
    },
    {
      "brand": "Apple",
      "model": "6",
      "price": 630.0,
      "imgSrc": "/img/products/smartPhone2.png",
      "description": `Colour: Black <br> Memory: 32GB`
    },
    {
      "brand": "Sony",
      "model": "Z",
      "price": 410.0,
      "imgSrc": "/img/products/smartPhone3.png",
      "description": `Colour: Black <br> Memory: 32GB`
    },
    {
      "brand": "Google",
      "model": "Pixel",
      "price": 550.0,
      "imgSrc": "/img/products/smartPhone4.png",
      "description": `Colour: White <br> Memory: 32GB`
    }
  ],
  "skis": [
    {
      "brand": "Fischer",
      "model": "RC4 Worldcup SC",
      "price": 2300.00,
      "imgSrc": "/img/products/ski1.png",
      "description": `Radius in m: 13m <br> Lengths in cm: 150, 155, 160, 165, 170`
    },
    {
      "brand": "Atomic",
      "model": "Redster Edge GS",
      "price": 2100.00,
      "imgSrc": "/img/products/ski2.png",
      "description": `Radius in m: 15 <br> Lengths in cm: 162, 169, 176, 183`
    },
    {
      "brand": "Fischer",
      "model": "RC4 Worldcup GS",
      "price": 2450,
      "imgSrc": "/img/products/ski3.png",
      "description": `Radius in m: 25 <br> Length in cm: 186`
    },
    {
      "brand": "Nordica",
      "model": "Dobermann GS",
      "price": 2600,
      "imgSrc": "/img/products/ski4.png",
      "description": `Radius in m: 25 <br> Length in cm: 184`
    }
  ]
};
